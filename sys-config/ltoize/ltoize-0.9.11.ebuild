...
pkg_preinst() {
	...
	if use clang; then
		dosym "${LTO_PORTAGE_DIR}/package.cflags/clang.conf" "${PORTAGE_CONFIGROOT%/}/etc/portage/package.cflags/clang.conf"
		dosym "${LTO_PORTAGE_DIR}/bashrc.d/44-lto-clang.sh" "${PORTAGE_CONFIGROOT%/}/etc/portage/bashrc.d/44-lto-clang.sh"
	fi
